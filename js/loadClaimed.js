  ...
  $.ajax({
    ...
    success: function(dataJson) {
      ...
      for(var i = 0; i < dataJson.length; i++) {
        ...
        newHtml += '</div><div class="card-footer"><button class="btn btn-dos ' + dataJson[i].claimed_by + '" type="button" name="userInfo">See ' + dataJson[i].claimed_by + ' profile</button></div>';
        newHtml += "</div>";
      }
